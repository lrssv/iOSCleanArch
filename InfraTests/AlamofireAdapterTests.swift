...
class AlamofireAdapterTests: XCTestCase {
    ...
    func test_post_should_complete_with_error_when_request_completes_with_non_200(){
        ...
        expectResult(.failure(.badRequest), when: (data: makeValidData(), response: makeHttpUrlResponse(statusCode: 450), error: nil))
        expectResult(.failure(.badRequest), when: (data: makeValidData(), response: makeHttpUrlResponse(statusCode: 499), error: nil))
        expectResult(.failure(.serverError), when: (data: makeValidData(), response: makeHttpUrlResponse(statusCode: 500), error: nil))
        expectResult(.failure(.serverError), when: (data: makeValidData(), response: makeHttpUrlResponse(statusCode: 550), error: nil))
